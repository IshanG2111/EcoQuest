...
async function runIngestWikiKG() {
  console.log('Connecting to MongoDB Atlas...');
  await connectDB();

  console.log(`Ingesting ${WIKIKG_NODES.length} WikiKG90Mv2 entities into MongoDB...`);
  for (const node of WIKIKG_NODES) {
    await EcoGraphNode.findOneAndUpdate({ id: node.id }, { $set: node }, { upsert: true, new: true });
  }

  console.log(`Ingesting ${WIKIKG_EDGES.length} WikiKG90Mv2 property edges into MongoDB...`);
  for (const edge of WIKIKG_EDGES) {
    await EcoGraphEdge.findOneAndUpdate({ id: edge.id }, { $set: edge }, { upsert: true, new: true });
  }

  console.log('✅ Successfully ingested WikiKG90Mv2 Stanford OGB Knowledge Graph data into EcoGraph MongoDB Atlas!');
  process.exit(0);
}

runIngestWikiKG().catch((err) => {
  console.error('Failed to ingest WikiKG90Mv2 dataset:', err);
  process.exit(1);
});
